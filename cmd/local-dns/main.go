...
func (c *cacheApp) parseAndValidateFlags() error {
	...
	flag.StringVar(&c.params.healthPort, "health-port", "8080", "port used by health plugin")
	flag.Parse()
	...
	if _, err := strconv.Atoi(c.params.healthPort); err != nil {
		return fmt.Errorf("Invalid healthcheck port specified - %q", c.params.healthPort)
	}
	return nil
}
